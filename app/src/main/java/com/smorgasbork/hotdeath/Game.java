...
public class Game extends Thread {
  ...
  public void dealHands() {
    ...
    String msg = String.format(getString(R.string.msg_dealing),
                               seatToString(m_dealer.getSeat()),
                               m_numCardsToDeal);
    promptUser(msg);
    ...
    if ((m_currPlayer.getHand()).hasValidCards(this)
      ...
      if (m_currPlayer.getWantsToPlayCard()) {
        ...
        if (m_currColor == Card.COLOR_WILD) {
          ...
          String msg = String.format(getString(R.string.msg_color_chosen),
                                     seatToString(m_currPlayer.getSeat()),
                                     colorToString(m_currColor));
          redrawTable();
          Log.d("HDU", msg);
          //PromptUser(msg);
        }
        ...
        if (m_currPlayer instanceof HumanPlayer) {
          String msg = String.format(getString(R.string.msg_player_draws_specific_card),
                                     seatToString(m_currPlayer.getSeat()),
                                     cardToString(m_currPlayer.getLastDrawn()));
          Log.d("HDU", msg);
          promptUser(msg);
        }
  ...
  boolean checkCard(Hand h, Card c, boolean interactive) {
    ...
    // 69 can be played on 6 or 9; likewise, 6 or 9 can be played on 69
		if ((id == Card.ID_YELLOW_69) && ((cvalue == 6) || (cvalue == 9))) {
			return true;
		}
		if ((cid == Card.ID_YELLOW_69) && ((value == 6) || (value == 9))) {
			return true;
		}

    // can play magic red 5 on any card
		if (cid == Card.ID_RED_5_MAGIC) {
			return true;
		}

    // the variants of D, S, and R
		if ((value == Card.VAL_D) && (cvalue == Card.VAL_D_SPREAD)) {
			return true;
		}
		if ((value == Card.VAL_D_SPREAD) && (cvalue == Card.VAL_D)) {
			return true;
		}
		if ((value == Card.VAL_R) && (cvalue == Card.VAL_R_SKIP)) {
			return true;
		}
		if ((value == Card.VAL_R_SKIP) && (cvalue == Card.VAL_R)) {
			return true;
		}
		if ((value == Card.VAL_S) && (cvalue == Card.VAL_S_DOUBLE)) {
			return true;
		}
		if ((value == Card.VAL_S_DOUBLE) && (cvalue == Card.VAL_S)) {
			return true;
		}
  ...
  public void finishRound(Player p) {
    ...
    redrawTable();
    String msg = String.format(getString(R.string.msg_declare_round_winner),
  ...
  */
  public int checkForDefender(Hand h) {
    ...
    if ((prevVal == Card.VAL_WILD_DRAWFOUR) && (prevID != Card.ID_WILD_HOS)) {
      for (int i = 0; i < h.getNumCards(); i++) {
        ...
        if (!defenderAlreadyThrown) {
          if ((val == Card.VAL_WILD_DRAWFOUR) && (prevID != Card.ID_WILD_MYSTERY) &&
              (id != Card.ID_WILD_MYSTERY)) {
            defenderCount++;
          }
        }
  ...
  public void handleSpecialCards() {
    ...
    if (currVal == Card.VAL_D_SPREAD) {
      ...
      for (int i = 0; i < 3; i++) {
        ...
        if (checkForShield(victim.getHand())) {
          ...
          String msg = String.format(getString(R.string.msg_has_blue_shield),
    ...
    if ((currID == Card.ID_RED_0_HD) && (m_penalty.getVictim() == m_currPlayer)) {
      ...
      String msg = String.format(getString(R.string.msg_holy_defender),
                                 seatToString(m_penalty.getVictim().getSeat()));
      promptUser(msg);
    }
    ...
    if ((currID == Card.ID_GREEN_3_AIDS)
      ...
      String msg = String.format(getString(R.string.msg_sharing_penalty),
                                 seatToString(m_penalty.getVictim().getSeat()));
      promptUser(msg);
    }
    ...
    if ((currID == Card.ID_BLUE_0_FUCKYOU)
      ...
      String msg = String.format(getString(R.string.msg_sending_penalty),
                                 seatToString(m_penalty.getVictim().getSeat()));
  ...
  public void assessPenalty() {
    ...
    if (m_penalty.getType() == Penalty.PENTYPE_CARD) {
      ...
      if (pVictim2 != null) {
        ...
        if (m_players[SEAT_SOUTH - 1] instanceof HumanPlayer) {
          msg = String.format(getString(R.string.msg_player_faceup),
                              seatToString(pVictim2.getSeat()));
          promptUser(msg);
        }
      }
      ...
      if (pVictim2 != null) {
        ...
        msg = String.format(getString(R.string.msg_player_ejected),
                            seatToString(pVictim2.getSeat()));
        redrawTable();
        promptUser(msg);
  ...
  void forceDraw(Player p, int numcards) {
    ...
    String msg = String.format(getString(R.string.msg_player_drawing),
                               seatToString(p.getSeat()),
                               numcards);
    Log.d("HDU", msg);
    promptUser(msg);
